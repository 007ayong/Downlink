(function initBackgroundShared(global) {
  ...
  function shouldCaptureByExt(config = {}, url, filename) {
    ...
    const filenameExt = extOf(filename);
    if (exts.includes('*')) return Boolean(urlExt || filenameExt);
    if (!exts.length) return Boolean(urlExt || filenameExt);
    return exts.includes(urlExt) || exts.includes(filenameExt);
  }
  ...
  function shouldCaptureByMime(config = {}, mime, url = '', filename = '') {
    ...
    if (normalized === 'application/octet-stream' || normalized === 'application/binary') {
      const exts = captureExts(config);
