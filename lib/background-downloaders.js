(function initBackgroundDownloaders(global) {
  ...
  const DOWNLOADERS = {
    ...
    abdownload: { label: (cfg) => cfg.externalLauncherName || 'AB Download' },
    neatdm: { label: () => 'NeatDM' },
  };
  ...
  function createClients({ getConfig, notify, onBeforeAria2Send, onAria2TaskQueued }) {
    ...
    async function getAria2GlobalStat() {
      return aria2Call('getGlobalStat');
    }
    ...
    async function sendToExternalLauncher(taskInfo, extraOpts = {}) {
      try {
        ...
        notify(`${getDownloaderLabel('abdownload')} 连接失败`, err.message);
    ...
    async function sendToNeatdm(taskInfo) {
      ...
      try {
        ...
        try { socket?.close(); } catch {}
        notify(`${getDownloaderLabel('neatdm')} 连接失败`, err.message);
        return { ok: false, error: err.message };
      }
    }
    ...
    async function sendToAria2(taskInfo, extraOpts = {}) {
      try {
        ...
        notify(`已发送到 ${getDownloaderLabel()}`, taskInfo.filename || taskInfo.url.slice(0, 80));
        return { ok: true, gid };
      } catch (err) {
        notify(`${getDownloaderLabel()} 连接失败`, err.message);
        return { ok: false, error: err.message };
      }
    }
